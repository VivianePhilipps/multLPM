...
RcppExport SEXP probaED(SEXP s0, SEXP st0, SEXP b0, SEXP Y0, SEXP X0, SEXP Xd0, SEXP D0, SEXP Xseuil0, SEXP nmes0, SEXP nv0, SEXP idx0, SEXP idiag0, SEXP ncor0, SEXP ny0, SEXP nalea0, SEXP ntr0, SEXP link0, SEXP nodes0, SEXP nRE0, SEXP nBM0) 
{ 
  ...
  double s = Rcpp::as<double>(s0); // landmark s
  double st = Rcpp::as<double>(st0); // tps landmark+horizon s+t
  ...
  RiDs.zeros(K,K);
  RiKDs.zeros(K,ni);
  Xdis.zeros(K,beta.size()); 
  Zdis.zeros(K,B.n_cols);    
  tdcors.zeros(K); 
  ...
  RiDst.zeros(2*K,2*K);
  RiKDst.zeros(2*K,ni);
  Xdist.zeros(2*K,beta.size()); 
  Zdist.zeros(2*K,B.n_cols);    
  tdcorst.zeros(2*K); 
  ...
  for(k=0; k<K; k++){	  // pr chq dimension  
    ...
    for(l=0; l<nv(k); l++){ // pr chq variable explicative de la dimension k
      ...
      if(idx(0,iidx+l)==1){ // si cette variable explicative est un effet fixe  
        ...
        //~X
        Xdis(jdcurrs,p) = Xdk(0,l); 
        //~X+
        Xdist(span(jdcurrst,jdcurrst+1),p) = Xdk(span(0,1),l);
        p += 1; 
      } 
      ...
      if(idx(2,iidx+l)==1){ // si cette variable explicative est un effet aleatoire 
        ...
        // ~Z
        Zdis(jdcurrs,q) = Xdk(0,l); 
        // ~Z+
        Zdist(span(jdcurrst,jdcurrst+1),q) = Xdk(span(0,1),l); 
        q += 1; 
      } 
      if(idx(3,iidx+l)==1){ // si cette variable explicative est un temps pour BM ou AR 
        ...
        //~W
        tdcors(jdcurrs) = Xdk(0,l); 
        //~W+
        tdcorst(span(jdcurrst,jdcurrst+1)) = Xdk(span(0,1),l); 
      } 
    }
    ...
    if(ncor(k)>0){ // si la dimension contient un processus d'autocorrelation
      ...
      if(ncor(k)==1){ // si il s'agit d'un BM 
        for(j1=0; j1<nik; j1++){ 
          ...
          // ~~R
          RiKDs(jdcurrs,jcurr+j1) = BM(kbm,kbm)*std::min(tcor(jcurr+j1),tdcors(jdcurrs));
          for(j3=0; j3<2; j3++){
            // ~~R+
        ...
        // ~R
        RiDs(jdcurrs,jdcurrs) = BM(kbm,kbm)*std::min(tdcors(jdcurrs),tdcors(jdcurrs)); 
        for(j1=0; j1<2; j1++){
          for(j2=0; j2<2; j2++){
            // ~R+
  ...
  Gam.zeros(1,K);
  Gam = trans(gamma*varD);
  
  //Gam.print("Gam =");
  
  GamPlus.zeros(2,2*K);
  GGPlus.eye(2,2);   // matrice identite
  ...
  seuil(0) *= dot(eta,varD);  // s'assurer que vecteur = 1 nbre revient a vecteur = (nbre,...,nbre)
  // pr les var expl dependantes du temps
  if(ndept>0){
      seuil(0) += dot(eta_dept,trans(Xseuil(0,span(0,ndept-1))));
  }
  ...
  seuilPlus.ones(2);
  // pr les var expl independantes du temps
  seuilPlus(span(0,1)) *= dot(eta,varD);  // s'assurer que vecteur = 1 nbre revient a vecteur = (nbre,...,nbre)
  // pr les var expl dependantes du temps
  if(ndept>0){
    for(j=0 ; j<2 ; j++){
  ...
  //~~~~~~~~~~~~~~~~~~~~~~~~//
  // FctRep du denominateur //
  //~~~~~~~~~~~~~~~~~~~~~~~~//
  
  dimPhi1 = 1;
  upper1.zeros(1); // borne superieure de la fct de repartition
  lower1.zeros(1); // borne inferieure de la fct de repartition
  infin1.zeros(1); // infin1.ones(Bs+1); 
  
  upper1 = seuil - Gam*muiD_Ks; // borne de l'integration  // point sur lequel on calcule la fct de repartition
  
  V1.zeros(1,1); 
  I1.eye(1,1);   // matrice identite
  ...
  correl1.zeros(1); // nbre de parametres de correlation a trouver = nbre d'elements de la matrice V1 - la moitie (symetrie) - diago (corr=1)
  
  
  upper11.zeros(dimPhi1); 
  upper11(0) = upper1(0)/pow(V1(0,0),0.5); // pow(x,p) = x^p  // seuil <- seuil/variance
  ...
  dimPhi2 = 2;
  upper2.zeros(2); // borne superieure de la fct de repartition
  lower2.zeros(2); // borne inferieure de la fct de repartition
  infin2.zeros(2); // infin.ones(Bst+1); 
  
  upper2 = seuilPlus - GamPlus*muiD_Kst; // borne de l'integration  
  
  V2.zeros(2,2); 
  I2.eye(2,2);   // matrice identite
  ...
  correl2.zeros(1); // nbre de parametres de correlation a trouver = nbre d'elements de la matrice V2 - la moitie (symetrie) - diago (corr=1)
