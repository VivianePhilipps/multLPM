...
btobtot <- function(Y,D,data,var.time,RE="block-diag",BM="diag",B,posfix,maxiter=0,nproc=1,verbose=FALSE,file="",pred=FALSE,breaks=NULL,delayed=TRUE,eps=c(0.0001,0.0001,0.001),bb)
    {
        ##### donnees Y #####
        K <- length(Y)
        ...
        if(D1[[3]]!=1)
            {
                form1 <- formula(paste("~",subject,"-1+",D1[3]))
                modmat1 <- model.matrix(form1,data=dataD)
        ...
        if(nbevt==2)
            {
                if(D2[[3]]!=1)
                    {
                        form2 <- formula(paste("~",subject,"-1+",D2[3]))
                        modmat2 <- model.matrix(form2,data=dataD)
        ...
        if(!missing(posfix))
            {
                if(missing(B)) B <- rep(0,length(btot))
        ...
        ## revenir aux prm varcov
        npmMM <- sum(npmtot)
        nef <- p-1
        ncontr <- ctr*(ny-1)
        nea <- q        
        nvc <- nea-1
        nvc[which(idiag==0)] <- nea[which(idiag==0)]*(nea[which(idiag==0)]+1)/2-1
                ...
                if(nea[k]>0)
                    {
                        if(nvc[k]>0)
                            {
                                vtmp <- VRE[imatB+1:nea[k],imatB+1:nea[k]]
                                if(idiag[k]==0)
                                    {
