...
uacv <- function(model,Y,D,data,var.time,RE="block-diag",BM="diag",B,posfix,breaks=NULL,delayed=TRUE)
    {
        ## verif arg
...
## a voir : demander 1 mesure a chaque Y ?
        ...
        ## gradients de vrais cond et tot
        npm <- length(b)
        matGrad <- matrix(0,nrow=2*ns,ncol=npm)
        for(k in 1:npm)
        {
            th <- max(0.000001,0.0001*abs(b[k]))

            bplus <- b
            bplus[k] <- bplus[k]+th

            bmoins <- b
            bmoins[k] <- bmoins[k]-th

            llplus <-  RloglikUACV(b0=bplus, bfix0=bfix, fix0=fix, Y0=Y, X0=X, Xd0=Xd, idD0=idD, D0=D, Xseuil0=Xseuil, nmes0=nmes, nv0=nv, idx0=idx, idiag0=idiag, ncor0=ncor, ny0=ny, nalea0=nalea, ntr0=ntr, link0=link, nodes0=nodes, epsY0=epsY, nRE0=nRE, nBM0=nBM, nbevt0=nbevt, idcause0=idcause, entreRetard0=entreRetard, nbTronc0=nbTronc)

            llmoins <- RloglikUACV(b0=bmoins, bfix0=bfix, fix0=fix, Y0=Y, X0=X, Xd0=Xd, idD0=idD, D0=D, Xseuil0=Xseuil, nmes0=nmes, nv0=nv, idx0=idx, idiag0=idiag, ncor0=ncor, ny0=ny, nalea0=nalea, ntr0=ntr, link0=link, nodes0=nodes, epsY0=epsY, nRE0=nRE, nBM0=nBM, nbevt0=nbevt, idcause0=idcause, entreRetard0=entreRetard, nbTronc0=nbTronc)
            
            matGrad[,k] <- -(llplus-llmoins)/(2*th)
        }


        ## variance des estimateurs
        varb <- matrix(0,nrow=npm,ncol=npm)
        varb[upper.tri(varb,diag=TRUE)] <- model$v
        varb <- t(varb)
        varb[upper.tri(varb,diag=TRUE)] <- model$v


        ## trace(H^(-1)K)
        prodGrad <- sapply(1:ns,function(i,mat) t(mat[i,,drop=FALSE])%*%mat[ns+i,,drop=FALSE],mat=matGrad,simplify=FALSE)
        sumGrad <- 1/(ns-1)*Reduce("+",prodGrad)

        trace <- sum(diag(varb%*%sumGrad))

        ## uacv
        res <- -1/ns*sum(vrais2i[1:ns])+trace
