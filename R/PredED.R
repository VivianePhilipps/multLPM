RprobaED <- function(s0, st0, b0, Y0, X0, Xd0, D0, Xseuil0, nmes0, nv0, idx0, idiag0, ncor0, ny0, nalea0, ntr0, link0, nodes0, nRE0, nBM0)
{
  .Call("probaED", s0, st0, b0, Y0, X0, Xd0, D0, Xseuil0, nmes0, nv0, idx0, idiag0, ncor0, ny0, nalea0, ntr0, link0, nodes0, nRE0, nBM0)
} # appel de fonction probaED codee en C++



## calcul de probabilite d'evenement ds intervalle de prediction pr un sujet specifique, cas discret ##
#' Probability of discrete event in the prediction interval ]s,s+t] for a specific subject
#'
...
#' @param D a list of two-sided formula defining the event part of the model. The left side should be longDiag(time0,time,indicator) with time0 the variable indicating entry time (which will only be used for delayed entry), time the repeated measurement times, indicator the repeated event indicator at each time
...
#' @return the probability of discrete event in the prediction interval ]s,s+t] for a specific subject
...
PredED <- function(num,s,t,model,Y,D,data,ID,var.time,RE="block-diag",BM="diag",B)
{
  #------------------------------#
  ...
  if(!length(grep("longDiag",D[[1]][2]))) stop("Left side of formula in D should be 'longDiag()'")
  ...
  ## evt absorbant et en temps discret
  ...
  dem <- gauche1[3] #d:indicateur event
  ...
  dataD1 <- data[,c(subject,entry,var.time[K+1],dem,setdiff(unique(c(varexplD,nomsX)),c(var.time,entry)))]
  ...
  dataD1_i <- dataD1_i[length(dataD1_i[,1]),,drop=FALSE] #derniere ligne de dataD1 du sujet (celle ac 1 si evt)
  ...
  #--------------------------------------#
  #- construction du dataframe du sujet -#
  #--------------------------------------#
  
  dataD1bis_i <- as.data.frame(matrix(NA,0,ncol(dataD1),dimnames=list(NULL,colnames(dataD1))))
  
  di1_i <- data.frame(rep(num,2),t01_i=rep(-Inf,2),t1_i=c(s,st),rep(0,2))
  if(ncol(dataD1)>4) #ie. si il y a des varexpl
  {
    for(k in 5:ncol(dataD1))
    {
      di1_i <- data.frame(di1_i,rep(dataD1[which(dataD1[,1]==num)[1],k],2)) # di1 = di1 + varExpl
    }
  }
  ...
  ## union des temps de mesure de l evt
  dataD <- dataD1
  ...
  D <- as.data.frame(unique(dataD[,1])) #sujet, indicateur event
  colnames(D) = "ID"
  
  
  ## nb de sujets (select au moins 1 mesure dem, non prevalents diag)       
  ...
  if(D1[[3]]!=1) # si il y a des VarExplSurv
  {
    ...
    modmat1 <- model.matrix(form1,data=dataD[which(!is.na(dataD[,dem])),])[,-1,drop=FALSE] #construit automatiquement les variables des interactions et des contrastes
    modmat1_i <- model.matrix(form1,data=dataD_i[which(!is.na(dataD_i[,dem])),])[,-1,drop=FALSE]
  ...
  ## tps s = borne inferieure de l intervalle de prediction
  s
  
  ## tps s+t = borne superieure de l intervalle de prediction
  st
  ...
  D_i <- NULL # si pas de VarExpSurv independante du temps
  D_i <- D[which(D[,1]==num),] # on ne garde que les VarExplSurv indep. du tps, on supprime son statut d event
  D_i <- unlist(D_i)
  ...
  res <- RprobaED(s0=s, st0=st, b0=b, Y0=Y_i, X0=X_i, Xd0=Xd_i, D0=D_i, Xseuil0=Xseuil_i, nmes0=nmes_i, nv0=nv, idx0=idx, idiag0=idiag, ncor0=ncor, ny0=ny, nalea0=nalea, ntr0=ntr, link0=link, nodes0=nodes, nRE0=nRE, nBM0=nBM)
